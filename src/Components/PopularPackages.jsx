...
function PopularPackages() {
  ...
  const nextHandler = () => {
    if (startIndex + ITEMS_PER_PAGE < packages.length) {
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8  ">
        {packages
          ...
          .map((item, index) => (
            <div
              key={index}
              className={`h-100 w-auto rounded-xl overflow-hidden ${item.bg}`}
            >
      ...
      <div className="flex justify-center gap-6 mt-10">
        <button
          ...
          className="px-6 py-2 rounded-full bg-gray-200 hover:bg-sky-400  disabled:opacity-40 cursor-pointer disabled:cursor-not-allowed"
        >
          ◀ Back
        </button>
        ...
        <button
          onClick={nextHandler}
          disabled={startIndex + ITEMS_PER_PAGE >= packages.length}
          className="px-6 py-2 rounded-full bg-gray-200 hover:bg-sky-400  disabled:opacity-40 cursor-pointer disabled:cursor-not-allowed"
        >
          Next ▶
        </button>
